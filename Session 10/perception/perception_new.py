...
class Perception:
    ...
    def run(self, perception_input: dict) -> dict:
        ...
        try:
            ...
            snapshot_type = perception_input.get("snapshot_type", "user_query")
            if snapshot_type == "step_result":
                step_output = perception_input.get("raw_input", "")
                failed = isinstance(step_output, str) and "Tool Failed" in step_output
                if not failed:
                    output["local_goal_achieved"] = True
                    if output.get("local_reasoning", "No local reasoning given.") == "No local reasoning given.":
                        output["local_reasoning"] = "Step produced usable intermediate results."
                else:
                    output["local_goal_achieved"] = False
                    output["local_reasoning"] = "Tool execution failed; no useful data."
